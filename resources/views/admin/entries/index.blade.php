					...
					<thead>
						<tr>
							...
							<th colspan="2" class="text-center">
								<a href="{{ route( 'entries.create', [ 'season' => $currentSeason->id ] ) }}" title="Add a entry" class="glyphicon glyphicon-plus"></a>
							</th>
						</tr>
					</thead>
					<tbody>
						@forelse( $entries as $entry )
							<tr>
								...
								<td>
									<a href="{{ route( 'entries.edit', [ 'seasons' => $entry->id ] ) }}">{{ $entry->driver->fullName }}</a>
								</td>
								<td>
									<span class="glyphicon glyphicon-{{ $entry->active ? 'ok text-success' : 'remove text-danger' }}"></span>
								</td>
								<td class="text-center">
									<a href="{{ route( 'entries.edit', [ 'seasons' => $entry->id ] ) }}" title="Edit this entry" class="glyphicon glyphicon-pencil"></a>
								</td>
								<td class="text-center">
									@if( $entry->results->isEmpty() and $entry->picks->isEmpty() )
										<a href="{{ route( 'entries.destroy', [ 'entries' => $entry->id ] ) }}" title="Delete this entry" class="glyphicon glyphicon-trash"></a>
									@else
										&nbsp;
									@endif
								</td>
